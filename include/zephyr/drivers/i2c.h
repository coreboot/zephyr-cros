...
static inline int i2c_write_dt_idx(const struct i2c_dt_spec *spec, uint8_t spec_idx,
				   const uint8_t *buf, uint32_t num_bytes)
{
	return i2c_write(spec->bus, buf, num_bytes, spec->addresses[spec_idx]);
}
...
static inline int i2c_reg_write_byte_dt_idx(const struct i2c_dt_spec *spec, uint8_t spec_idx,
					    uint8_t reg_addr, uint8_t value)
{
	return i2c_reg_write_byte(spec->bus, spec->addresses[spec_idx], reg_addr, value);
}
...
/**
 * @brief Detect which address a device responds on.
 *
