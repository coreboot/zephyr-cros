...
static bch_poly_t chunk_remainder(const struct bch_def *def, const uint8_t *chunk, size_t len)
{
...
void bch_generate(const struct bch_def *bch, const uint8_t *chunk, size_t len, uint8_t *ecc)
{
	pack_poly(bch, chunk_remainder(bch, chunk, len), ecc);
}

int bch_verify(const struct bch_def *bch, const uint8_t *chunk, size_t len, const uint8_t *ecc)
{
...
static void poly_add(gf13_elem_t *dst, const gf13_elem_t *src, gf13_elem_t c, int shift)
{
	int i;
...
static gf13_elem_t syndrome(const struct bch_def *bch, const uint8_t *chunk, size_t len,
			    bch_poly_t remainder, gf13_elem_t x)
{
	gf13_elem_t y = 0;
	gf13_elem_t t = 1;
	int i;
...
static void berlekamp_massey(const gf13_elem_t *s, int N, gf13_elem_t *sigma)
{
...
void bch_repair(const struct bch_def *bch, uint8_t *chunk, size_t len, uint8_t *ecc)
{
