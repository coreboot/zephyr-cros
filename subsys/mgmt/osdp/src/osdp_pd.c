...
static void pd_decode_command(struct osdp_pd *pd, uint8_t *buf, int len)
{
	...
	case CMD_MFG:
		...
		else if (ret > 0) { /* App wants to send a REPLY_MFGREP to the CP */
	...
	case CMD_KEYSET:
		...
		if (buf[pos] != 1 || buf[pos + 1] != 16) {
			LOG_ERR(TAG "Keyset invalid len/type: %d/%d", buf[pos], buf[pos + 1]);
			break;
		}
		cmd = osdp_cmd_alloc(pd);
		if (cmd == NULL) {
	...
	if (ret != 0) {
		LOG_ERR(TAG "Invalid command structure. CMD: %02x, Len: %d", pd->cmd_id, len);
		pd->reply_id = REPLY_NAK;
...
static int pd_receve_packet(struct osdp_pd *pd)
{
	...
	if (IS_ENABLED(CONFIG_OSDP_PACKET_TRACE)) {
		 ...
		 */
		if (pd->rx_buf_len > 8 && pd->rx_buf[6] != CMD_POLL && pd->rx_buf[8] != CMD_POLL) {
...
void osdp_update(struct osdp *ctx)
{
	...
	case OSDP_PD_STATE_IDLE:
		...
		if (ret == -1 || ((pd->rx_buf_len > 0 || ISSET_FLAG(pd, PD_FLAG_SC_ACTIVE)) &&
