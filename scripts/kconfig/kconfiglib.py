...
class Kconfig(object):
    ...
    def _finalize_sym(self, sym):
        ...
        if len(sym.configdefaults) > 0:
            inserted = 0
            for (idx, defaults) in sym.configdefaults:
                for d in defaults:
                    # Determine the final dependency of the symbol by or'ing
                    # together all prompts. sym.direct_deps cannot be used as
                    # it does not include prompt dependencies (BAR in below):
                    # config FOO
                    #     bool "FOO" if BAR
                    complete_deps = self.n
                    for n in sym.nodes:
                        if n.prompt:
                            complete_deps = self._make_or(complete_deps, n.prompt[1])
                    ...
                    default = (d[0], self._make_and(complete_deps, d[1]))
